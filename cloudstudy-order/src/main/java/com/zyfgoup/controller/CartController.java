...
public class CartController {
    ...
    public Result addCart(@RequestBody Map<String,String> map, HttpServletRequest request){
        ...
        Cart one = cartService.getOne(new QueryWrapper<Cart>().eq("courseId", cart1.getCourseId()).eq("userId", cart1.getUserId()));
        if(one !=null){
    ...
    public Result getListByUser(HttpServletRequest request){
        ...
        List<Cart> list = cartService.list(new QueryWrapper<Cart>().eq("userId", userVo.getId()));
    ...
    public Result removeByCourseId(@PathVariable("id") String id){
        boolean b = cartService.remove(new QueryWrapper<Cart>().eq("courseId",id));
        if(!b){
            return Result.fail("删除失败");
        }
        return Result.succ("操作成功");
