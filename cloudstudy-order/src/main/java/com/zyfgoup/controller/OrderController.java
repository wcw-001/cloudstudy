...
public class OrderController {
    ...
    public Result get(@PathVariable("current") Long current,@PathVariable("size") Long size,@RequestBody Map<String,String> searchObj) {
        ...
        wrapper.eq(StrUtil.isNotBlank(courseId), "courseId", courseId);
        wrapper.eq(StrUtil.isNotBlank(teacherName), "teacherName", teacherName);
        ...
        if (StrUtil.isNotBlank(begin)) {
            ...
            wrapper.apply("UNIX_TIMESTAMP(createTime) >= UNIX_TIMESTAMP('" + startTime + "')");
        }
    ...
    public Result delete(@PathVariable("id") String id){
        ...
        boolean save = orderService.update(new UpdateWrapper<Order>().eq("id",id).set("isDeleted",1));
        if(!save){
    ...
    public Result saveOrder(@PathVariable String courseId, HttpServletRequest request) throws InterruptedException {
        ...
        //已经有这个订单了 不需要重新生成
        Order one = orderService.getOne(new QueryWrapper<Order>().eq("courseId", courseId).eq("userId", userVo.getId())
                .eq("isDeleted", 0));
        if(one !=null){
    ...
    public Result listOrderNo(@PathVariable("orderNo") String orderNo, HttpServletRequest request) {
        ...
        if(list.size()>0){
            //删除购物车
            cartService.remove(new QueryWrapper<Cart>().eq("userId",userVo.getId()));
            return Result.succ(true);
        }
        return Result.succ(false);
    }
    ...
    public Result remove(@PathVariable String orderId, HttpServletRequest request) {
        ...
        .eq("userId",userVo.getId()).set("isDeleted",1));
        if(result){
