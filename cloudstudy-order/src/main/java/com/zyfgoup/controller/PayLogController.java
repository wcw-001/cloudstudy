...
public class PayLogController {
    ...
    public Result createQrCode(@PathVariable("id") String id){
        try {
            ...
            m.put("mchId", ConstantWxUtils.WX_PAY_PARTNER);
            m.put("nonceStr", WXPayUtil.generateNonceStr());
            m.put("body", order.getCourseTitle()); //课程标题
            m.put("outTradeNo", order.getId()); //如果是在购物车里生成的订单 又在订单里单独购买 不能再使用orderNo
            m.put("totalFee", order.getTotalFee().multiply(new BigDecimal(100)).longValue() + "");
            m.put("spbillCreateIp", "127.0.0.1");
            //m.put("notifyUrl", "http://guli.shop/api/order/weixinPay/weixinNotify\n");
            m.put("notifyUrl", ConstantWxUtils.WX_PAY_NOTIFYURL);
            m.put("tradeType", "NATIVE");
            ...
            map.put("resultCode", resultMap.get("resultCode"));  //返回二维码操作状态码
            map.put("codeUrl", resultMap.get("codeUrl"));        //二维码地址
    ...
    public Result queryPayStatus(@PathVariable String orderNo)  {
        ...
        if(map.get("tradeState").equals("SUCCESS")) {//支付成功
            //添加记录到支付表，更新订单表订单状态
