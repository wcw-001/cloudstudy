...
public class AdminController {
    ...
    public Map<String,Object> getByName(@PathVariable("name") String name){
        ...
        //未被删除的
         Admin admin = adminService.getOne(new QueryWrapper<Admin>().eq("name",name).eq("isDeleted","0"));
    ...
    public Result getPageList(@PathVariable("current") Long current,@PathVariable("size") Long size,
        ...
        if(StrUtil.isBlank(name)){
            if(!redisTemplate.hasKey(RedisKey.ALL_Admin_MEMBER)) {
                List<Admin> list = adminService.list(new QueryWrapper<Admin>().eq("isDeleted",0));
    ...
    public Result remove(@PathVariable("id")String id){
        ...
        boolean update = adminService.update(new UpdateWrapper<Admin>().eq("id", id).set("isDeleted", 1));
        if(!update){
    ...
    public Result freeze(@RequestBody Map<String,String>map){
        ...
        boolean update = adminService.update(new UpdateWrapper<Admin>().eq("id", id).set("isDisabled", 1));
        if(!update){
    ...
    public Result unfreeze(@RequestBody Map<String,String>map){
        ...
        boolean update = adminService.update(new UpdateWrapper<Admin>().eq("id", id).set("isDisabled", 0));
        if(!update){
    ...
    public Result toAssign(@PathVariable String userId) {
        ...
        List<AclAdminRole> existsList = adminRoleService.list(new QueryWrapper<AclAdminRole>().eq("userId", userId).select("roleId"));
    ...
    public Result doAssign(@RequestParam String userId,@RequestParam String[] roleIds) {
        ...
        //先删除已分配的
        boolean remove = adminRoleService.remove(new QueryWrapper<AclAdminRole>().eq("userId", userId));
