...
public class AclRoleController {
    ...
    public Result delete(@PathVariable("id") String id){
        ...
        boolean update = aclRoleService.update(new UpdateWrapper<AclRole>().eq("id", id).set("isDeleted", 1));
        if(!update){
    ...
    public Result getPageList(@PathVariable("current") Long current,@PathVariable("size") Long size,
        ...
        if(StrUtil.isBlank(roleName)){
            if(!redisTemplate.hasKey(RedisKey.ALL_ROLE)) {
                List<AclRole> list = aclRoleService.list(new QueryWrapper<AclRole>().eq("isDeleted",0));
        ...
        wrapper.like(StrUtil.isNotBlank(roleName),"roleName",roleName);
