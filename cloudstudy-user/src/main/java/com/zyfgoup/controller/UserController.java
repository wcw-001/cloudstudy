...
public class UserController {
    ...
    public Result delete(@PathVariable("userid") String userid){
        boolean b = userService.update(new UpdateWrapper<User>().set("isDeleted", 1).eq("id", userid));
        if(b){
    ...
    public Result disadbled(String id){
        boolean b = userService.update(new UpdateWrapper<User>().eq("id", id).set("isDisabled", 1));
        if(b){
